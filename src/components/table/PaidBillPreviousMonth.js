  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Calculate paid counts for previous month
        ...
        // Calculate total counts for previous month (paid + unpaid)
        ...
        const finalPaidCounts = allWards.reduce((acc, ward) => {
        ...
        setWardPaidCounts(finalPaidCounts);
        setPreviousMonthTotalBills(finalTotalCounts);
            ...
            <TableBody>
              {allWards.map((ward, index) => (
                <StyledRow key={ward} index={index}>
                  ...
                  <StyledCell>{previousMonthTotalBills[ward] || 0}</StyledCell>
                </StyledRow>
              ))}
            </TableBody>
          </Table>
