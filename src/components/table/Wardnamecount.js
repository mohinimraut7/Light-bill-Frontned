...
const Wardnamecount = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${baseUrl}/getConsumers?limit=5000`) // increase limit if backend supports it
      .then((res) => res.json())
      .then((data) => {
        // ✅ Access the consumers array safely
        const consumersArray = data?.consumers || [];

        // ✅ Count ward-wise consumers
        const counts = consumersArray.reduce((acc, consumer) => {
          const ward = consumer.ward;
          if (ward) {
            acc[ward] = (acc[ward] || 0) + 1;
          }
