...
export const fetchConsumersSuccess=(data)=>({
    type:FETCH_CONSUMERS_SUCCESS,
    payload:data
})
...
export const fetchConsumers = (page = 1, limit = 50, cnId, wardName) => {
  return async (dispatch) => {
    dispatch(fetchConsumersRequest());
    try {
      ...
      // Add cnId to query if provided
      if (cnId) {
        queryParams += `&consumerNumber=${encodeURIComponent(cnId)}`;
      }
      
      // Add wardName to query if provided
      if (wardName) {
        queryParams += `&ward=${encodeURIComponent(wardName)}`;
      }
      
      const response = await axios.get(`${baseUrl}/getConsumers?${queryParams}`);
      dispatch(fetchConsumersSuccess(response.data));
    } catch (error) {
      dispatch(fetchConsumersFailure(error.message));
    }
