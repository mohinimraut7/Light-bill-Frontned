...
export const SET_TOTAL_BILLS_COUNT = 'SET_TOTAL_BILLS_COUNT';
...
// total count action (new)
export const setTotalBillsCount = (total) => ({
  type: SET_TOTAL_BILLS_COUNT,
  payload: total
});
...
// export const fetchBills = (page = 0, pageSize = 10, setData = () => {}) => {
//   return async (dispatch) => {
//     dispatch(fetchBillsRequest());
//     try {
//       const response = await axios.get(`${baseUrl}/getBills?page=${page}&pageSize=${pageSize}`);
//       const { bills, total } = response.data;

//       dispatch(fetchBillsSuccess({ bills, total }));

//       // 👇 Preserve old pattern
//       setData(bills);
...
// };
// ===============================================
// actions/billActions.js
// export const fetchBills = (page = null, pageSize = null, setData = () => {}) => {
//   return async (dispatch) => {
//     dispatch(fetchBillsRequest());
//     try {
//       const url = page !== null && pageSize !== null
//         ? `${baseUrl}/getBills?page=${page}&pageSize=${pageSize}`
//         : `${baseUrl}/getBills`;

//       const response = await axios.get(url);

//       // Flexible response handling
//       const bills = response.data.bills || response.data; // support both paginated and old response
//       const total = response.data.total || bills.length;
...
export const fetchBills = (page, pageSize) => {
  return async (dispatch) => {
    dispatch(fetchBillsRequest());

    try {
      let url = `${baseUrl}/getBills`;
      ...
      // ✅ Support both { bills: [], total: 100 } and plain array []
      const { bills, total } = Array.isArray(response.data)
        ? { bills: response.data, total: response.data.length }
        : response.data;

      dispatch(fetchBillsSuccess(bills));

      // Optional: if you use total count in pagination
      dispatch({
        type: 'SET_TOTAL_BILLS_COUNT',
        payload: total,
      });
...
// export const fetchBills = (page = 1, limit = 10) => {
//   return async (dispatch) => {
//     dispatch(fetchBillsRequest());
//     try {
//       const response = await axios.get(`${baseUrl}/getBills?page=${page}&limit=${limit}`);
...
//       // ✅ Dispatch total separately using new action
//       dispatch(setTotalBillsCount(total));
//     } catch (error) {
//       dispatch(fetchBillsFailure(error.message));
//     }
//   };
