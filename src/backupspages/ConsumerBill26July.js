...
const ConsumerBill = () => {
  ...
  const total = useSelector((state) => state.bills.total);
...
const currentMonthYear = `${currentMonth}-${currentYear}`;
  ...
  }, [dispatch,data,page, pageSize]);


//   useEffect(() => {
//   const fetchBills = async () => {
//     try {
//       const res = await axios.get(`/api/bills?page=${page}&limit=${pageSize}`);
//       setRows(res.data.bills); // set fetched bills
//       setTotal(res.data.total); // total count from backend
//     } catch (error) {
...
//   fetchBills();
// }, [page, pageSize]); // 👈 Dependency array: page or pageSize change => fetch
...
//     try {
//       const res = await axios.get(`/api/bills?page=${page}&limit=${pageSize}`);
//       setBillRows(res.data.bills);
//       setBillTotalCount(res.data.total);
//     } catch (error) {
//       console.error('Error fetching bills:', error);
//     }
//   };
