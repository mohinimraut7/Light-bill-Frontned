...
const ConsumerBill = () => {
  ...
  const fetchBillsWithPagination = useCallback(async (page, pageSize, filters = {}) => {
    ...
    const paginationParams = {
      ...
      consumerNumber: filters.consumerNumber || '',
      ward: filters.ward || '',
      monthYear: filters.monthYear || '', // Optional - empty string if not selected
      userRole: user?.role,
      userWard: user?.ward
    };
  ...
  useEffect(() => {
    const filters = {
      consumerNumber: searchDebounce,
      ward: wardName,
      monthYear: selectedMonthYear // Can be empty string - optional filter
    };
  ...
  // Updated getFilteredBills function - removed client-side filtering since backend handles it
  const getFilteredBills = () => {
    return bills; // Backend already filters based on user role and ward
  };
